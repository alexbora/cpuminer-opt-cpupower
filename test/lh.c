 ...
 * @file        : lh
 * @created     : Joi Apr 18, 2024 20:00:25 EEST
 */
...
#define __libc_ptf_call_always(FUNC, ARGS) PTHFCT_CALL(ptr_##FUNC, ARGS)
#else
#define PTFAVAIL(NAME) (NAME != NULL)
#define __libc_ptf_call(FUNC, ARGS, ELSE) __libc_maybe_call(FUNC, ARGS, ELSE)
...
#else
#define __libc_lock_lock(NAME) __libc_lock_lock_fn(&(NAME))
...
#else
#define __libc_lock_lock_recursive(NAME) __libc_lock_lock_recursive_fn(&(NAME))
#endif /* __OPTION_EGLIBC_BIG_MACROS */
#else
#define __libc_lock_lock_recursive(NAME) \
...
#else
#define __libc_lock_trylock(NAME) __libc_lock_trylock_fn(&(NAME))
#endif /* __OPTION_EGLIBC_BIG_MACROS */
#else
...
#else
#define __libc_lock_unlock(NAME) __libc_lock_unlock_fn(&(NAME))
#endif /* __OPTION_EGLIBC_BIG_MACROS */
#else
...
#define __libc_once(ONCE_CONTROL, INIT_FUNCTION)                \
  do {                                                          \
    if (PTFAVAIL(__pthread_once))                               \
      __libc_ptf_call_always(__pthread_once,                    \
                             (&(ONCE_CONTROL), INIT_FUNCTION)); \
...
#define __libc_cleanup_region_start(DOIT, FCT, ARG)         \
  {                                                         \
    ...
    if (DOIT) {                                             \
      ...
      if (_avail) {                                         \
        __libc_ptf_call_always(_pthread_cleanup_push_defer, \
                               (&_buffer, FCT, ARG));       \
      } else {                                              \
...
__extern_inline void __libc_cleanup_routine(struct __pthread_cleanup_frame *f) {
  if (f->__do_it) f->__cancel_routine(f->__cancel_arg);
}
...
extern int __register_atfork(void (*__prepare)(void), void (*__parent)(void),
                             void (*__child)(void), void *__dso_handle);
...
extern int __pthread_setspecific(pthread_key_t __key, __const void *__pointer);
...
extern int __pthread_atfork(void (*__prepare)(void), void (*__parent)(void),
                            void (*__child)(void));
...
#endif
weak_extern(BP_SYM(__pthread_mutex_init)) weak_extern(
    BP_SYM(__pthread_mutex_destroy)) weak_extern(BP_SYM(__pthread_mutex_lock))
    weak_extern(BP_SYM(__pthread_mutex_trylock)) weak_extern(BP_SYM(
        __pthread_mutex_unlock)) weak_extern(BP_SYM(__pthread_mutexattr_init))
        weak_extern(BP_SYM(__pthread_mutexattr_destroy)) weak_extern(
            BP_SYM(__pthread_mutexattr_settype))
            weak_extern(BP_SYM(__pthread_rwlock_init)) weak_extern(
                BP_SYM(__pthread_rwlock_destroy))
                weak_extern(BP_SYM(__pthread_rwlock_rdlock)) weak_extern(
                    BP_SYM(__pthread_rwlock_tryrdlock))
                    weak_extern(BP_SYM(__pthread_rwlock_wrlock)) weak_extern(
                        BP_SYM(__pthread_rwlock_trywrlock))
                        weak_extern(BP_SYM(__pthread_rwlock_unlock)) weak_extern(
                            BP_SYM(__pthread_key_create))
                            weak_extern(BP_SYM(__pthread_setspecific)) weak_extern(
                                BP_SYM(__pthread_getspecific))
                                weak_extern(BP_SYM(__pthread_once)) weak_extern(
                                    __pthread_initialize)
                                    weak_extern(__pthread_atfork) weak_extern(
                                        BP_SYM(_pthread_cleanup_push_defer))
                                        weak_extern(BP_SYM(
                                            _pthread_cleanup_pop_restore))
                                            weak_extern(
                                                BP_SYM(pthread_setcancelstate))
#else
